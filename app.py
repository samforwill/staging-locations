...
st.set_page_config(page_title="SL Map", layout="wide")
st.title("Staging Location Mapping")
...
with c4:
    total_sls = len(filtered_ooc) + len(filtered_regular)
    st.metric(
        "SL Points in Filter", 
        f"{total_sls} ({len(filtered_ooc)} OOC)"
    )
...
for _, point in filtered_ooc.iterrows():
    ...
    <div style='width: 250px; padding: 8px;'>
        <b style='font-size: 14px; color: #e67e00;'>OOC Location</b><br><br>
        <b>Proposed Region/Turf:</b> {point.get('proposed_regionturf', 'N/A')}<br>
        <b>Proposed Location:</b> {point.get('location', 'N/A')}<br>
        <b>Actual Region:</b> {point.get('Region', 'N/A')}<br>
        <b>Actual Turf:</b> {point.get('fo_name', 'N/A')}<br>
        <b>Precinct:</b> {point.get('van_precinct_name', 'N/A')}<br>
        <b>County:</b> {point.get('county', 'N/A')}
    </div>
    """
    ...
    folium.Marker(
        ...
        icon=folium.Icon(color='orange', icon='', prefix='')
    ).add_to(m)
...
for _, point in filtered_regular.iterrows():
    popup_html = f"""
    <div style='width: 250px; padding: 8px;'>
        <b style='font-size: 14px; color: #6a0dad;'>Staging Location</b><br><br>
        <b>Proposed Region/Turf:</b> {point.get('proposed_regionturf', 'N/A')}<br>
        <b>Proposed Location:</b> {point.get('Location', 'N/A')}<br>
        <b>Actual Region:</b> {point.get('Region', 'N/A')}<br>
        <b>Actual Turf:</b> {point.get('fo_name', 'N/A')}<br>
    ...
    folium.Marker(
        location=[point['latitude'], point['longitude']],
        popup=folium.Popup(popup_html, max_width=300),
        icon=folium.Icon(color='purple', icon='', prefix='')
    ).add_to(m)
...
if bin_labels and len(filtered_metrics) > 0:
    ...
    <p><b>SLS Points</b></p>
    <p><i class="fa fa-map-marker" style="color:purple; font-size:14px;"></i> Staging Location</p>
    <p><i class="fa fa-map-marker" style="color:orange; font-size:14px;"></i> OOC SL</p>
    '''
